...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \draw[->, >=Stealth](.5,1.)--++(0,.5)node[above]{$y$};
    \end{tikzpicture}
    \caption{
        中性子の経路。
...
実験には反射率が安定している波長帯を与える入射角が望ましい。
\eqref{eq: theoretical: delta phi g}の角度依存性、使用する中性子ビームの波長帯、先行研究\cite{Fujiie_2024}でのセットアップを合わせて考慮し、事前シミュレーションでは$\theta=1.05^\circ$を採用した。
