...
// constexpr double V_per45 = (297.4e-09 + 104.4e-09)/2.* 1.6022e-19;

constexpr double V_ge = 94.1e-09 * 1.6022e-19;

constexpr double D_per45 = 154.8e-10 / delta_D; // 厚さが1/0.93倍になっていたことに気付く
...
constexpr int Nroop_lambda = (int)((lambda_max - lambda_min) / d_lambda);
...
void refforlambda()
{
    ...
    double lambda[Nroop_lambda]; // 波長
    double theta = 1.05;         // 入射角
    double rad = theta * PI / 180.;
    double R[3][Nroop_lambda]; // 反射率
    double hantei[2][Nroop_lambda] = {0.};

    for (int j = 0; j < Nroop_lambda; j++)
    {
    ...
    TGraph *graph1 = new TGraph(Nroop_lambda, lambda, R[0]);
    TGraph *graph1_d = new TGraph(Nroop_lambda, lambda, R[2]);
    ...
    TGraph *graph2 = new TGraph(Nroop_lambda, lambda, R[1]);
    TGraph *nhantei2 = new TGraph(Nroop_lambda, lambda, hantei[1]);
