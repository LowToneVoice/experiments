...
Int_t *MakeNikiRPMT(char* DataFileName)
{
  ...
  while(fscanf(fp, "%d,%d,%d,%lld,%d",&board,&ch,&e,&t,&dummy)!=EOF){
    ...
    //if time is later than sett+dt, then Fill data in tuple
      ...
      if((sRpmt.iPulse[0]+sRpmt.iPulse[1])!=0)  RPMT_X = Double_t(sRpmt.iPulse[1])/Double_t(sRpmt.iPulse[0]+sRpmt.iPulse[1]);
      else  RPMT_X = -1.;
      if((sRpmt.iPulse[2]+sRpmt.iPulse[3])!=0)  RPMT_Y = Double_t(sRpmt.iPulse[3])/Double_t(sRpmt.iPulse[2]+sRpmt.iPulse[3]);
      else  RPMT_Y = -1.;
