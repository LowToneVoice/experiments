...
int main(int argc, char *argv[])
{
  ...
  if(argc==1 || argc>3){
    std::cerr<<"Usage: ./MakeRPMT [DataFileName] [Option Make Both:0(default), Only RPMT:1, Only single:2]" <<std::endl;
    return -1;
  } else {
    os <<argv[1];
  ...
  MakeRPMT(DataFileName); //for RPMT 
...
Int_t *MakeRPMT(char* DataFileName)
{
  Int_t i=0,nlines=0;
