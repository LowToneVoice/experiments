...
int main(int argc, char *argv[]){
  ...
  double FillingTime        = T_FillingFixed * 1000000.;
  ...
  int n_storage = (int) sizeof(T_storage) / sizeof(T_storage[0]);
  int ii;
  double StorageTime = T_storage[0] * 1000000.;

  clock_t start;
  time_t timer;
  
  FILE *fp;
  if((fp=fopen(logfilename_cell,"a"))==NULL){
  ...
  printf("# Duration from (RV:Filling, CV:Open) to (RV:Filling, CV:Close): %f s.\n",FillingTime / 1000000.);
  printf("# Duration from (RV:Filling, CV:Close) to (RV:Detection, CV:Close): %f s.\n",PreparateRVTime / 1000000.);
  printf("# Duration from (RV:Filling, CV:Close) to (RV:Detection, CV:Open) is ...\n");
  for(ii=0;ii<n_storage;ii++){
    printf("#    Case %d:  %f s.\n", ii+1, T_storage[ii]);
  }
  printf("#    Total case number of storage time: %d\n", n_storage);
  ...
  fprintf(fp,"# Duration from (RV:Filling, CV:Open) to (RV:Filling, CV:Close): %f s.\n",FillingTime / 1000000.);
  fprintf(fp,"# Duration from (RV:Filling, CV:Close) to (RV:Detection, CV:Close): %f s.\n",PreparateRVTime / 1000000.);
  fprintf(fp,"# Duration from (RV:Filling, CV:Close) to (RV:Detection, CV:Open) is ...\n");
  for(ii=0;ii<n_storage;ii++){
    fprintf(fp,"#    Case %d:  %f s.\n", ii+1, T_storage[ii]);
  }
  fprintf(fp,"#    Total case number of storage time: %d\n", n_storage);
  ...
  system("../GV10");
  time(&timer);
  printf("GV10 (RV:Detection, CV:Open) is performed @ %s",ctime(&timer));
  fprintf(fp,"GV10 (RV:Detection, CV:Open) is performed @ %s",ctime(&timer));
  start=clock();
  ...
  while(1){
    if(i==0){
      ...
      if( (run==0 && (clock()-start) >= FirstCleaningTime) || (run!=0&& (clock()-start)>=DAQAndCleaningTime ) ){
        ...
        system("../GV00");
        time(&timer);
        printf("GV00 (RV:Filling, CV:Open) is performed @ %s",ctime(&timer));
        fprintf(fp,"GV00 (RV:Filling, CV:Open) is performed @ %s",ctime(&timer));
        fflush(fp);
        i++;
        start=clock();
      }//(font adjustment)
      ...
      if( (clock()-start) >= PreparateRVTime ){
        ...
        i++;
        start=clock();
        StorageTime = T_storage[j] * 1000000.;
